<x-slot name="header">
    <div class="flex items-center justify-between">
        <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">
            {{ __('navbar.settings') }} / {{ __('navbar.register') }} / {{ __('reports.bp_classification') }} / {{ __('navbar.consult') }}
        </h1>
    </div>
</x-slot>
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <table class="w-full table-fixed divide-y divide-gray-200 dark:divide-gray-800">
            ...
            <tr class="text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">
                ...
                <th class="px-4 py-3">{{ __('reports.parent_code') }}</th>
                <th class="px-4 py-3">{{ __('reports.name') }}</th>
                <th class="px-4 py-3">{{ __('reports.config_name') }}</th>
                <th class="px-4 py-3">{{ __('reports.tree') }}</th>
                <th class="px-4 py-3">{{ __('reports.company') }}</th>
                <th class="px-4 py-3">{{ __('reports.side') }}</th>
                <th class="px-4 py-3">{{ __('reports.section') }}</th>
                <th class="px-4 py-3">{{ __('reports.sort_order') }}</th>
            ...
            @forelse($response as $res)
                ...
                >
                    ...
                    <td class="px-4 py-3">{{ __("classification.{$res->side}") ?? $res->side }}</td>
                    <td class="px-4 py-3">{{ __("classification.{$res->section}") ?? $res->section }}</td>
                    <td class="px-4 py-3">{{ $res->sort_order }}</td>
